...
import { Bell, BookMarked, Search, Settings, LogOut, PenSquare, User, Loader2, X, Menu } from 'lucide-react';
import DOMPurify from 'dompurify';
import { Skeleton } from './ui/skeleton';

const API_URL = "http://127.0.0.1:5000";

const Navbar = ({ showWriteStory = true }) => {
        ...
        <div>
          ...
          {searchResults.posts.length > 0 ? (
            searchResults.posts.map(post => (
              <div key={post.id} className="mb-4 hover:bg-gray-800 p-3 rounded-lg transition duration-150 ease-in-out">
                <Link 
                  to={`/post/${post.id}`} 
                ...
                <p className="text-gray-400 text-sm mb-2" 
                   dangerouslySetInnerHTML={{
                     __html: DOMPurify.sanitize(truncateText(stripHtml(post.body), 150))
                   }}
                />
        ...
        <div>
          ...
          {searchResults.users.length > 0 ? (
            searchResults.users.map(user => (
              <Link key={user.id} to={`/profile/${user.id}`} className="flex items-center mb-4 hover:bg-gray-800 p-3 rounded-lg transition duration-150 ease-in-out">
                <Avatar className="mr-3">
                  <AvatarImage src={user.profile_pic ? `${API_URL}/uploads/${user.profile_pic}` : null} />
                  <AvatarFallback className="bg-blue-500">{user.username[0].toUpperCase()}</AvatarFallback>
                </Avatar>
                <div>
  ...
  return (
      ...
      <div className="container mx-auto px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex md: items-center space-x-4 flex-grow justify-center">
            <div className="relative w-full max-w-md md:max-w-lg lg:max-w-2xl" ref={searchRef}>
              <form onSubmit={handleSearch} className="relative md:w-[80%] md:ml-8 lg:w-[90%]">
                <Input
          ...
          <nav className="flex items-center space-x-4">
            {isAuthenticated ? (
                ...
                <DropdownMenu onOpenChange={setIsDropdownOpen}>
                  <DropdownMenuTrigger asChild>
                    <div className="relative cursor-pointer hidden md:block">
                      <Avatar>
                        <AvatarImage src={`${API_URL}/uploads/${userData.profile_pic}`} alt="User" />
                        <AvatarFallback className="bg-gradient-to-br from-blue-500 to-purple-600 text-white">
                          {userData.username ? userData.username[0].toUpperCase() : 'U'}
                        </AvatarFallback>
                      </Avatar>
                      <Badge className="absolute -top-1 -right-1 px-1.5 py-0.5 text-xs bg-gradient-to-r from-blue-500 to-purple-600 text-white">3</Badge>
                    </div>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent className="w-64 bg-gray-800 border-gray-700 text-gray-100 p-2 rounded-lg shadow-lg z-50 relative">
                    <DropdownMenuLabel className="text-blue-400 font-bold text-lg pb-2">{userData.firstName} {userData.lastName}</DropdownMenuLabel>
              ...
              <Button onClick={handleLogin} className="hidden md:flex bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white">
                Login
              </Button>
            )}
            <Button
          ...
          >
            <div className="container mx-auto px-4 py-4 space-y-4">
              ...
              {isAuthenticated ? (
                <>
                  {showWriteStory && (
                    <Link to="/create-post" className="block" onClick={() => setIsMobileMenuOpen(false)}>
                  ...
                  <Button className="w-full text-left justify-start" variant="ghost" onClick={() => { navigate('/notifications'); setIsMobileMenuOpen(false); }}>
                    <Bell className="mr-2 h-5 w-5" />
                    <span>Notifications</span>
                  </Button>
                  <Button className="w-full text-left justify-start" variant="ghost" onClick={() => { handleBookmarksClick(); setIsMobileMenuOpen(false); }}>
                    <BookMarked className="mr-2 h-5 w-5" />
                    <span>Bookmarks</span>
                  </Button>
                  <Button className="w-full text-left justify-start" variant="ghost" onClick={() => { handleSettingsClick(); setIsMobileMenuOpen(false); }}>
                    <Settings className="mr-2 h-5 w-5" />
                    <span>Settings</span>
                  </Button>
                  ...
                  <Button className="text-right justify-end bg-gray-700" variant="ghost" onClick={() => { handleLogout(); setIsMobileMenuOpen(false); }}>
                    <LogOut className="mr-2 h-5 w-5" />
                    <span>Logout</span>
                  </Button>
                  </div>
                ...
                <Button onClick={() => { handleLogin(); setIsMobileMenuOpen(false); }} className="w-full bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white">
                  Login
                </Button>
              )}
            </div>
    ...
    {isDropdownOpen && (
    <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm z-40" onClick={() => setIsDropdownOpen(false)} />
    )}
    </>
  );
};
